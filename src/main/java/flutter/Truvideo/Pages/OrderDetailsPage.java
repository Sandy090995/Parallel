...
public class OrderDetailsPage extends UtilityClass {
   ...
   @iOSXCUITFindBy(accessibility = "Technician" )
	...
	public boolean checkAllImportantTextsArePresent() {
		if (getOrderNumber().getText().contains(AddOrderPage.orderNumber) && customerBlock.isDisplayed() && serviceAdvisorBlock.isDisplayed()
				&& technicianBlock.isDisplayed()) {
			scrollDown();
			if(lastMessagesBlock.isDisplayed() && mediaGallery.isDisplayed()
