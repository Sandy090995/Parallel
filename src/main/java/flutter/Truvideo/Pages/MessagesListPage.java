	        ...
	        for (WebElement statusObject : statusListAll) {
	            ...
	            try {
	                if (messagingScreen.getConversationStatus_Open().isDisplayed() &&
	                	...
	                	isOwnMessage=true;
	                	 if (isAndroid()) {
		                    	loginUserMessage = sentMessageByLoginUser(UserListPage.userName).getAttribute("content-desc");
		                    } else {
		                    	loginUserMessage = sentMessageByLoginUser(UserListPage.userName).getAttribute("label");
		                    }
		                    log.info("In My Active Filter RO is of Login User and status is Open: " + loginUserMessage);
	                	}else {
	                		isOwnMessage=false;
	                	}
	                ...
	                log.error("Exception while processing conversation status", e);
	            } finally {	              
	                ...
	                if (sentMessageByLoginUser(UserListPage.userName).isDisplayed()) {
	                	isOwnMessage=true;
	                	 if (isAndroid()) {
		                    ...
		                    log.info("In My Active Filter RO is of Login User & message is " + loginUserMessage);
	                	}else {
	                		isOwnMessage=false;
	                	}
	                } catch (NoSuchElementException e) {
